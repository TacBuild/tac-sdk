...
export class TacSdk {
    ...
    private async getJettonProxyAddress(): Promise<string> {
        return await this.settings.getAddressSetting('JettonProxyAddress');
    }

    private async getCrossChainLayerAddress(): Promise<string> {
        return await this.settings.getAddressSetting('CrossChainLayerAddress');
    }

    private async getJettonMinterCode(): Promise<Cell | null> {
        return await this.settings.getCellSetting('JETTON_MINTER_CODE');
    }

    private async getJettonWalletCode(): Promise<Cell | null> {
    ...
    private buildEvmArgumentsCell(transactionLinker: TransactionLinker, evmProxyMsg: EvmProxyMsg): Cell {
    ...
    private getJettonTransferPayload(transactionLinker: TransactionLinker, jettonProxyAddress: string, jettonData: JettonTransferData, evmProxyMsg: EvmProxyMsg): Cell {
        const l2Data = this.buildEvmArgumentsCell(transactionLinker, evmProxyMsg);
        const forwardAmount = 0.2;
        return JettonWallet.transferMessage(jettonData.jettonAmount, jettonProxyAddress, jettonData.fromAddress, forwardAmount, jettonData.tonAmount, l2Data, transactionLinker.queryId);
    };

    private getJettonBurnPayload(transactionLinker: TransactionLinker, jettonData: JettonBurnData, evmProxyMsg: EvmProxyMsg): Cell {
        const l2Data = this.buildEvmArgumentsCell(transactionLinker, evmProxyMsg);
        return JettonWallet.burnMessage(jettonData.jettonAmount, jettonData.notificationReceieverAddress, jettonData.tonAmount, l2Data, transactionLinker.queryId);
    }

    private getTonTransferPayload(transactionLinker: TransactionLinker, evmProxyMsg: EvmProxyMsg, tonAmount?: number): Cell {
        const l2Data = this.buildEvmArgumentsCell(transactionLinker, evmProxyMsg);
    ...
    private async detectOpType(jetton: JettonOperationGeneralData, cclAddress: Address): Promise<JettonOpType> {
        ...
        if (!protocolJettonMinterCode) throw new Error('unexpected empty wrappers minter code. Make sure the settings contract is valid.');
        await sleep(this.delay * 1000);

        const {code: givenMinterCodeBOC} = await this.tonClient.getContractState(address(jetton.tokenAddress));
        if (!givenMinterCodeBOC) throw new Error('unexpected empty contract code of given wrappers.');
        ...
        if (!protocolJettonWalletCode) throw new Error('unexpected empty wrappers wallet code. Make sure the settings contract is valid.');
        await sleep(this.delay * 1000);
    ...
    async sendCrossChainJettonTransaction(jettons: JettonOperationGeneralData[], evmProxyMsg: EvmProxyMsg, sender: SenderAbstraction): Promise<{
